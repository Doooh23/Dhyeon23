"""
LSTM·ARIMA·Transformer 시계열 모델을 활용한 주식 매수 타점 예측 프로그램
"""
...
.main-title{display:none}.metric-card,.info-card{background:rgba(255,255,255,.88)!important;border:1px solid var(--line)!important;border-radius:20px!important;box-shadow:0 14px 34px rgba(15,23,42,.06)!important}.good{color:var(--green);font-weight:900}.bad{color:var(--red);font-weight:900}.warn{color:#71717a;font-weight:900}hr{border-color:var(--line)}
...
st.markdown("""
<style>
:root{--tv-bg:#0b1120;--tv-panel:#111827;--tv-panel2:#172033;--tv-line:rgba(148,163,184,.22);--tv-text:#e5e7eb;--tv-muted:#94a3b8;--card-blue:#27272a;--card-green:#27272a;--card-red:#71717a}
...
.terminal-card{background:linear-gradient(180deg,#18181b,#111827)!important;color:#e5e7eb!important;border:1px solid rgba(148,163,184,.22)!important;border-radius:24px!important;padding:16px 18px!important;box-shadow:0 22px 56px rgba(15,23,42,.22)!important}.terminal-card .muted{color:#94a3b8;font-size:12px}
.badge{display:inline-flex;align-items:center;gap:6px;padding:6px 10px;border-radius:999px;font-size:12px;font-weight:900;letter-spacing:-.01em}.badge-green{background:rgba(18,184,134,.13);color:#27272a;border:1px solid rgba(18,184,134,.22)}.badge-red{background:rgba(250,82,82,.13);color:#71717a;border:1px solid rgba(250,82,82,.22)}.badge-blue{background:rgba(49,130,246,.13);color:#27272a;border:1px solid rgba(49,130,246,.22)}.badge-gray{background:rgba(148,163,184,.13);color:#64748b;border:1px solid rgba(148,163,184,.22)}
.kpi-grid{display:grid;grid-template-columns:repeat(5,minmax(0,1fr));gap:12px;margin:14px 0}.kpi-card{background:rgba(255,255,255,.86);border:1px solid rgba(148,163,184,.24);border-radius:20px;padding:14px 15px;box-shadow:0 14px 34px rgba(15,23,42,.06)}.kpi-label{font-size:12px;color:#64748b;font-weight:800}.kpi-value{font-size:21px;font-weight:950;letter-spacing:-.04em;color:#18181b;margin-top:4px}.kpi-sub{font-size:11px;color:#94a3b8;margin-top:4px}
.progress-row{margin:10px 0 12px}.progress-head{display:flex;justify-content:space-between;font-size:12px;font-weight:800;color:#cbd5e1;margin-bottom:5px}.progress-track{height:8px;border-radius:999px;background:rgba(148,163,184,.20);overflow:hidden}.progress-bar{height:8px;border-radius:999px;background:linear-gradient(90deg,#27272a,#27272a)}
...
[data-testid="stSidebar"] hr{border-color:#3f3f46!important}
.stButton>button{
...
.section-card,.asset-header,.trade-panel,.signal-terminal,.chart-shell,
[data-testid="stMetric"],.kpi-card,.plan-item,.stPlotlyChart{
...
.section-card{padding:14px 16px!important;margin:10px 0!important}
.asset-header{padding:18px!important;margin:12px 0!important}
.terminal-card{
  background:#27272a!important;
...
.badge-green,.badge-red,.badge-blue,.badge-gray{
...
.plan-grid{gap:7px!important}
.stPlotlyChart{padding:4px!important}
...
/* Plotly가 터치 제스처를 직접 처리하도록 허용 */
.stPlotlyChart .js-plotly-plot{touch-action:none!important}
.progress-number{
  display:flex;justify-content:space-between;align-items:center;
  padding:9px 11px;margin:6px 0 8px 0;
  border:1px solid var(--mono-line);border-radius:9px;background:#fff;
  color:var(--mono-ink);font-size:13px;font-weight:650;
}
.progress-number strong{font-size:18px;color:#09090b}
[data-testid="stProgressBar"] div[role="progressbar"]>div{background:#27272a!important}
@media(max-width:900px){
  .block-container{padding-left:.7rem;padding-right:.7rem}
  .asset-price{font-size:26px!important}
}
</style>
""", unsafe_allow_html=True)
    ...
    try:
        ...
        if r.status_code == 200:
            ...
            for item in quotes:
                ...
                if quote_type in {"EQUITY", "ETF"} and symbol and not symbol.endswith("-USD"):
                    return symbol
    except Exception:
        pass
    return upper if not upper.endswith("-USD") else None
...
def walk_forward_predictions(
    ...
    if X is None:
        _emit_progress(progress_callback, 1.0, "백테스트 데이터가 부족합니다")
        return pd.DataFrame()
    _emit_progress(progress_callback, 0.02, "백테스트 표본 준비 완료")
    ...
    total_chunks = max(1, math.ceil(len(eval_positions) / chunk_size))
    for chunk_index, chunk_start in enumerate(range(0, len(eval_positions), chunk_size)):
        chunk_positions = eval_positions[chunk_start: chunk_start + chunk_size]
        chunk_base = chunk_index / total_chunks
        _emit_progress(progress_callback, chunk_base, f"검증 구간 {chunk_index + 1}/{total_chunks} 준비")
        ...
        if "LSTM" in names:
            _emit_progress(progress_callback, (chunk_index + 0.15) / total_chunks, f"검증 구간 {chunk_index + 1}/{total_chunks} · LSTM 학습")
        ...
        if "Transformer" in names:
            _emit_progress(progress_callback, (chunk_index + 0.45) / total_chunks, f"검증 구간 {chunk_index + 1}/{total_chunks} · Transformer 학습")
            model_prediction_vectors.append(
                deep_predict_cached(
                    "Transformer",
                    X_train,
        ...
        if "ARIMA" in names:
            _emit_progress(progress_callback, (chunk_index + 0.70) / total_chunks, f"검증 구간 {chunk_index + 1}/{total_chunks} · ARIMA 계산")
            arima_vector = []
        ...
        _emit_progress(progress_callback, (chunk_index + 1) / total_chunks, f"검증 구간 {chunk_index + 1}/{total_chunks} 완료")

    _emit_progress(progress_callback, 1.0, "Walk-forward 예측 완료")
...
def latest_prediction(
    ...
    _emit_progress(progress_callback, 0.02, "학습 데이터 구성")
    X, idxs, y = build_dataset(df, lookback, horizon, market_score)
    ...
    if "LSTM" in names:
        _emit_progress(progress_callback, 0.18, "LSTM 학습 및 예측")
        predictions.append(
            float(
                deep_predict_cached(
                    "LSTM",
    ...
    if "Transformer" in names:
        _emit_progress(progress_callback, 0.45, "Transformer 학습 및 예측")
        predictions.append(
            float(
                deep_predict_cached(
                    "Transformer",
    ...
    if "ARIMA" in names:
        _emit_progress(progress_callback, 0.72, "ARIMA 예측")
        predictions.append(
            float(
                arima_forecast_cached(
    ...
    _emit_progress(progress_callback, 0.88, "모델 결과 결합")
    ...
    _emit_progress(progress_callback, 1.0, "최신 예측 완료")
    return row
...
def _add_compatible_candles(fig, frame, x_values, multiplier, inc_color, dec_color, row=1, col=1, name="가격"):
    """Plotly Candlestick 스키마 차이를 피하는 Scatter 기반 OHLC 캔들 렌더러."""
    if frame is None or frame.empty:
        return

    work = frame.loc[:, ["open", "high", "low", "close"]].copy()
    for price_col in ["open", "high", "low", "close"]:
        work[price_col] = pd.to_numeric(work[price_col], errors="coerce") * float(multiplier)

    x_series = pd.Series(list(x_values), index=work.index)
    valid = work.notna().all(axis=1) & x_series.notna()
    work = work.loc[valid]
    x_series = x_series.loc[valid]
    if work.empty:
        return

    up_mask = work["close"] >= work["open"]

    def segment_values(mask, first_col, second_col):
        xs, ys = [], []
        for idx in work.index[mask]:
            xv = x_series.loc[idx]
            xs.extend([xv, xv, None])
            ys.extend([float(work.at[idx, first_col]), float(work.at[idx, second_col]), None])
        return xs, ys

    # 꼬리(저가~고가)
    up_wick_x, up_wick_y = segment_values(up_mask, "low", "high")
    down_wick_x, down_wick_y = segment_values(~up_mask, "low", "high")
    if up_wick_x:
        fig.add_trace(
            go.Scatter(
                x=up_wick_x,
    ...
    # 몸통(시가~종가). 선 굵기로 캔들 몸통을 표현한다.
    up_body_x, up_body_y = segment_values(up_mask, "open", "close")
    down_body_x, down_body_y = segment_values(~up_mask, "open", "close")
    if up_body_x:
        fig.add_trace(
            go.Scatter(
                x=up_body_x,
                y=up_body_y,
                mode="lines",
                line=dict(color=inc_color, width=7),
                hoverinfo="skip",
                showlegend=False,
                name=f"{name} 상승",
            ),
    ...
    if down_body_x:
        fig.add_trace(
            go.Scatter(
                ...
                y=down_body_y,
                mode="lines",
                line=dict(color=dec_color, width=7),
                hoverinfo="skip",
                showlegend=False,
                name=f"{name} 하락",
            ),
    ...
    # 투명 마커를 이용해 OHLC 툴팁을 제공한다.
    hover_data = np.column_stack([
        work["open"].to_numpy(dtype=float),
    ...
    fig.add_trace(
        go.Scatter(
            ...
            hovertemplate=(
                ...
                "종가 %{customdata[3]:,.2f}<extra></extra>"
            ),
            showlegend=False,
            name=name,
        ),
...
def make_price_chart(df, pred_df=None, buys=None, sells=None, min_prob=0.6, min_expected_net=0.01):
    ...
    if globals().get("show_support_resistance", True):
        if "resistance60" in dfp.columns:
            fig.add_trace(go.Scatter(x=x, y=dfp["resistance60"] * mult, line=dict(color="rgba(82,82,91,.60)", width=1, dash="dash"), name="저항60", hoverinfo="skip"), row=1, col=1)
        if "support60" in dfp.columns:
            fig.add_trace(go.Scatter(x=x, y=dfp["support60"] * mult, line=dict(color="rgba(161,161,170,.65)", width=1, dash="dash"), name="지지60", hoverinfo="skip"), row=1, col=1)
    ...
    if pred_df is not None and not pred_df.empty and {"idx", "prob_up", "expected_net"}.issubset(pred_df.columns):
        ...
        if not cand.empty:
            ...
            if valid_idx:
                fig.add_trace(go.Scatter(x=valid_idx, y=(df.loc[valid_idx, "low"] * 0.975 * mult), mode="markers", marker=dict(symbol="circle", size=9, color="#27272a", opacity=.72, line=dict(width=1, color="#fff")), name="예측 후보", hovertemplate="예측 후보<extra></extra>"), row=1, col=1)
...
st.sidebar.markdown(f"<h3 style='color:{C['text']}'>설정</h3>", unsafe_allow_html=True)
if st.sidebar.button("모의투자 초기화", use_container_width=True):
    reset_local_portfolio()
    st.rerun()

st.sidebar.markdown("---")
st.sidebar.markdown(f"<h3 style='color:{C['text']}'>표시 설정</h3>", unsafe_allow_html=True)
...
st.sidebar.markdown("---")
st.sidebar.markdown(f"<h3 style='color:{C['text']}'>차트 설정</h3>", unsafe_allow_html=True)
chart_visual_theme = "미니멀 라이트"
...
st.sidebar.markdown("---")
st.sidebar.markdown(f"<h3 style='color:{C['text']}'>매수 점수 가중치</h3>", unsafe_allow_html=True)
...
st.sidebar.markdown("---")
st.sidebar.markdown(f"<h3 style='color:{C['text']}'>리스크 관리</h3>", unsafe_allow_html=True)
...
def _emit_progress(callback, value, message):
    if callback is None:
        return
    callback(float(np.clip(value, 0.0, 1.0)), str(message))


def _mapped_progress(callback, start, end):
    if callback is None:
        return None
    span = max(0.0, float(end) - float(start))
    return lambda value, message: callback(float(start) + span * float(np.clip(value, 0.0, 1.0)), message)


def create_progress_display(title):
    """진행 막대와 숫자 퍼센트를 함께 갱신하는 콜백을 만든다."""
    bar = st.progress(0)
    number_box = st.empty()

    def update(value, message="처리 중"):
        value = float(np.clip(value, 0.0, 1.0))
        pct = int(round(value * 100))
        bar.progress(pct)
        number_box.markdown(
            f'<div class="progress-number"><span>{message}</span><strong>{pct}%</strong></div>',
            unsafe_allow_html=True,
        )

    update(0.0, title)
    return update, bar, number_box
...
def render_prediction_card(latest, market):
    ...
    <div class="terminal-card">
      ...
      {_pct_bar('상승확률', latest['prob_up'], '#27272a')}
      {_pct_bar('종합점수', latest.get('final_score', 0), '#27272a')}
      {_pct_bar('AI 점수', latest.get('ml_score', 0), '#52525b')}
      {_pct_bar('기술점수', latest.get('tech_score', 0), '#71717a')}
      {_pct_bar('리스크 점수', latest['risk_score'], '#71717a')}
    </div>
    '''
...
def render_trade_plan(df, latest):
    ...
    <div class="trade-panel" style="padding:14px 15px;margin-top:12px;">
      <div style="font-weight:950;color:#18181b;margin-bottom:8px;">주문 계획 시뮬레이터</div>
      <div class="plan-grid">
        <div class="plan-item"><div class="plan-label">예상 진입가</div><div class="plan-value">{fmt_curr(close)}</div></div>
        <div class="plan-item"><div class="plan-label">손절 기준</div><div class="plan-value" style="color:#71717a;">{fmt_curr(stop)} / -{stop_pct*100:.1f}%</div></div>
        <div class="plan-item"><div class="plan-label">목표가</div><div class="plan-value" style="color:#27272a;">{fmt_curr(target)} / +{risk_cfg.take_profit_pct*100:.1f}%</div></div>
...
def render_metrics(metrics, acc, acc_n):
    ...
    for label, value, sub in cards:
        color = '#27272a' if ('+' in value and label != 'MDD') else ('#71717a' if label in ['MDD','평균손실'] and '-' in value else '#18181b')
...
def analyze_symbol(symbol, interval, data_range, scanner_fast=False, progress_callback=None):
    _emit_progress(progress_callback, 0.02, "주가 데이터 불러오는 중")
    ...
    if df_raw is None or len(df_raw) < minimum_rows:
        _emit_progress(progress_callback, 1.0, "데이터가 부족합니다")
        return None, name, None, None, None, [], pd.DataFrame(), [], [], {}, 0.0, 0, None

    _emit_progress(progress_callback, 0.14, "기술지표 계산")
    df = calculate_indicators(df_raw)
    _emit_progress(progress_callback, 0.22, "시장 추세 확인")
    market = get_market_regime(symbol, interval, data_range)
    market_ok = market["ok"] if use_market_filter else True

    latest_end = 0.84 if scanner_fast else 0.47
    latest = latest_prediction(
        ...
        progress_callback=_mapped_progress(progress_callback, 0.24, latest_end),
    )

    if scanner_fast:
        _emit_progress(progress_callback, 0.90, "최신 신호 정리")
        ...
        _emit_progress(progress_callback, 0.48, "Walk-forward 백테스트 시작")
        pred_df = walk_forward_predictions(
            ...
            arima_order=arima_order,
            progress_callback=_mapped_progress(progress_callback, 0.48, 0.88),
        )
        _emit_progress(progress_callback, 0.90, "매매 규칙 백테스트")
...
def render_asset_tab(default_list, default_input):
    asset_type = "stock"
    label = "주식"
    st.markdown('<div class="section-card"><h3 style="margin:0;color:#18181b;font-weight:800;">주식 분석</h3></div>', unsafe_allow_html=True)
    c1, c2, c3 = st.columns([3, 4, 3])
    with c1:
        method = st.radio("주식 검색 방식", ["목록에서 고르기", "직접 입력"], key="stock_method", label_visibility="collapsed")
    with c2:
        if method == "목록에서 고르기":
            user_input = st.selectbox("주식 선택", default_list, key="stock_select")
        else:
            user_input = st.text_input("주식명 또는 티커", default_input, key="stock_input").strip()
    with c3:
        time_label = st.selectbox("차트 주기", list(TIMEFRAME_MAP.keys()), index=0, key="stock_time")
    interval, data_range = TIMEFRAME_MAP[time_label]
    symbol = search_ticker_by_name(user_input)
    if not symbol:
        st.warning("주식 종목명 또는 티커를 입력해 주세요.")
        return

    result_key = "analysis_result_stock"
    signature_key = "analysis_signature_stock"
    mode_key = "analysis_run_mode_stock"
    current_signature = (
    ...
    with run_col2:
        run_full = st.button(
            ...
            use_container_width=True,
            key="run_full_stock",
            help="과거 구간을 반복 검증합니다. 신경망 모델은 시간이 오래 걸릴 수 있습니다.",
        )

    if run_quick or run_full:
        mode_text = "빠른 예측" if run_quick else "정밀 백테스트"
        update_progress, _, number_box = create_progress_display(f"{mode_text} 준비")
        try:
            result = analyze_symbol(
                symbol,
                interval,
                data_range,
                scanner_fast=run_quick,
                progress_callback=update_progress,
            )
            update_progress(1.0, f"{mode_text} 완료")
            st.session_state[result_key] = result
            st.session_state[signature_key] = current_signature
            st.session_state[mode_key] = "quick" if run_quick else "full"
        except Exception as exc:
            update_progress(1.0, "실행 중 오류 발생")
    ...
    if st.session_state.get(signature_key) != current_signature or result_key not in st.session_state:
        st.info("종목과 모델을 선택한 뒤 예측 버튼을 누르세요.")
        return
    ...
    if df is None:
        st.error("데이터가 부족하거나 불러오지 못했습니다. 일봉 또는 더 긴 기간을 선택해 주세요.")
        return
    ...
    with left:
        ...
        if run_mode == "full":
            ...
            if latest is None:
                ...
                st.info("빠른 예측은 최신 시점 1건만 계산합니다. 백테스트 지표는 정밀 백테스트에서 계산됩니다.")
    with right:
    ...
    if run_mode == "full":
        ...
        if trades:
            ...
            h["성공"] = h["성공"].map(lambda x: "성공" if x else "실패")
            st.dataframe(h.iloc[::-1], use_container_width=True, hide_index=True)
        else:
            st.info("정밀 백테스트가 완료됐지만 현재 조건을 통과한 거래가 없습니다.")

    st.markdown("---")
    st.markdown("### 주식 모의투자")
    share_key = "stock_shares"
    balance_key = "stock_balance"
    log_key = "stock_trade_log"
    ...
    with a3:
        amount = st.number_input(
            ...
            min_value=0.0001,
            value=1.0,
            step=1.0 if is_korean_ticker(symbol) else 0.1,
            key="manual_amt_stock",
        )
    b1, b2 = st.columns(2)
    with b1:
        if st.button("매수", use_container_width=True, key="buy_stock"):
            ...
            if st.session_state[balance_key] >= total:
                ...
                st.error("잔액이 부족합니다.")
    with b2:
        if st.button("매도", use_container_width=True, key="sell_stock"):
...
def render_scanner():
    ...
    if scan_type == "기본 목록":
        ...
        raw = st.text_area("주식 티커를 쉼표로 입력", "AAPL,NVDA,TSLA,MSFT,005930.KS")
        base_list = [x.strip().upper() for x in raw.split(",") if x.strip() and not x.strip().upper().endswith("-USD")]
    if not base_list:
        st.warning("스캔할 주식 티커를 입력해 주세요.")
        return
    ...
    if st.button("스캔 실행", use_container_width=True):
        ...
        for k, sym in enumerate(selected):
            def symbol_progress(local_value, message, index=k, ticker=sym):
                ...
                update_progress(overall, f"{ticker} · {message}")
...
if page == "종목 분석":
    render_asset_tab(
        ["애플", "삼성전자", "SK하이닉스", "테슬라", "엔비디아", "마이크로소프트", "구글", "네이버", "카카오", "현대차"],
        "애플",
    )
else:
    render_scanner()
